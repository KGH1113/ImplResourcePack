...
use crate::{
    ...
    models::{
        ...
        KeySoundOutputBackendPersist, KeyViewerKind, OverlayBounds, OverlayResizeAnchor,
        OverlayWindowLayout, SettingsDiff, SettingsState,
    },
    services::{css_watcher::CssWatcher, obs_bridge::ObsBridgeService, settings::SettingsService},
};

const HAND_OVERLAY_LABEL: &str = "hand-overlay";
const FOOT_OVERLAY_LABEL: &str = "foot-overlay";
const OVERLAY_KINDS: [KeyViewerKind; 2] = [KeyViewerKind::Hand, KeyViewerKind::Foot];
...
const OVERLAY_PAIR_GAP: f64 = 16.0;

fn overlay_label(kind: KeyViewerKind) -> &'static str {
    match kind {
        KeyViewerKind::Hand => HAND_OVERLAY_LABEL,
        KeyViewerKind::Foot => FOOT_OVERLAY_LABEL,
    }
}

fn overlay_title(kind: KeyViewerKind) -> &'static str {
    match kind {
        KeyViewerKind::Hand => "DM Note - Hand Overlay",
        KeyViewerKind::Foot => "DM Note - Foot Overlay",
    }
}

fn overlay_layout(state: &AppStoreData, kind: KeyViewerKind) -> &OverlayWindowLayout {
    match kind {
        KeyViewerKind::Hand => &state.overlay_windows.hand,
        KeyViewerKind::Foot => &state.overlay_windows.foot,
    }
}

fn overlay_layout_mut(state: &mut AppStoreData, kind: KeyViewerKind) -> &mut OverlayWindowLayout {
    match kind {
        KeyViewerKind::Hand => &mut state.overlay_windows.hand,
        KeyViewerKind::Foot => &mut state.overlay_windows.foot,
    }
}
...
pub struct AppState {
    ...
    hand_overlay_initializing: Arc<AtomicBool>,
    foot_overlay_initializing: Arc<AtomicBool>,
...
impl AppState {
    fn overlay_initializing_flag(&self, kind: KeyViewerKind) -> Arc<AtomicBool> {
        match kind {
            KeyViewerKind::Hand => self.hand_overlay_initializing.clone(),
            KeyViewerKind::Foot => self.foot_overlay_initializing.clone(),
        }
    }

    fn existing_overlay_windows(&self, app: &AppHandle) -> Vec<WebviewWindow> {
        OVERLAY_KINDS
            .iter()
            .filter_map(|kind| app.get_webview_window(overlay_label(*kind)))
            .collect()
    }
    ...
    pub fn initialize(store: AppStore) -> Result<Self> {
        ...
        Ok(Self {
            ...
            hand_overlay_initializing: Arc::new(AtomicBool::new(false)),
            foot_overlay_initializing: Arc::new(AtomicBool::new(false)),
    ...
    pub fn initialize_runtime(&self, app: &AppHandle) -> Result<()> {
        ...
        if !snapshot.obs_mode_enabled {
            self.ensure_overlay_windows(app)?;
        }
        ...
        if snapshot.developer_mode_enabled {
            ...
            for overlay in self.existing_overlay_windows(app) {
    ...
    pub fn bootstrap_payload(&self) -> BootstrapPayload {
        ...
        BootstrapPayload {
            ...
            selected_viewer_tabs: state.selected_viewer_tabs.clone(),
    ...
    pub fn obs_hide_overlay(&self, app: &AppHandle) {
        ...
        *self.obs_previous_overlay_visible.write() = Some(was_visible);
        ...
        for window in self.existing_overlay_windows(app) {
            if let Err(e) = window.destroy() {
                log::warn!("[ObsBridge] {} destroy 실패: {}", window.label(), e);
    ...
    pub fn obs_restore_overlay(&self, app: &AppHandle) {
        ...
        match prev {
            ...
            Some(false) => {
                ...
                if let Err(e) = self.ensure_overlay_windows(app) {
    ...
    pub fn set_overlay_visibility(&self, app: &AppHandle, visible: bool) -> Result<()> {
        ...
        if visible {
            // 두 창을 모두 준비한 뒤 표시해 부분적으로 열린 상태를 방지한다.
            let windows = self.ensure_overlay_windows(app)?;
            let snapshot = self.store.snapshot();
            for window in &windows {
                if let Err(err) = (|| -> Result<()> {
                    ...
                    for candidate in &windows {
                        let _ = hide_overlay_window(candidate);
                    }
                    return Err(err);
                }
            }
    ...
    pub fn resize_overlay(
        ...
        let window = app
            .get_webview_window(overlay_label(viewer_kind))
            .ok_or_else(|| anyhow!("{} window is not open", overlay_label(viewer_kind)))?;
        let anchor = anchor
        ...
        let initializing = self
            .overlay_initializing_flag(viewer_kind)
            .swap(false, Ordering::SeqCst);
        if initializing {
            // store에서 저장된 위치를 가져와 사용 (빌더 position이 무시될 수 있으므로)
            let snapshot = self.store.snapshot();
            if let Some(stored) = overlay_layout(&snapshot, viewer_kind).bounds.as_ref() {
                new_x = stored.x;
                new_y = stored.y;
            }
            ...
            if let Some(offset) = content_top_offset {
                if offset.is_finite() {
                    let _ = self.store.update(|state| {
                        overlay_layout_mut(state, viewer_kind).last_content_top_offset =
                            Some(offset);
                    })?;
                }
            }
            ...
            if let Some(offset) = content_top_offset {
                if offset.is_finite() {
                    let snapshot = self.store.snapshot();
                    let previous = overlay_layout(&snapshot, viewer_kind)
                        .last_content_top_offset
                        .unwrap_or(offset);
        ...
        let _ = self.store.update(|state| {
            let layout = overlay_layout_mut(state, viewer_kind);
            layout.bounds = Some(bounds.clone());
            layout.bounds_are_logical = true;
        })?;
    ...
    pub fn start_keyboard_hook(&self, app: AppHandle) -> Result<()> {
        ...
        let reader_handle = thread::Builder::new()
            ...
            let mut keys_state_emit_count: u64 = 0;
                ...
                let mut hand_overlay_window =
                    app_handle.get_webview_window(HAND_OVERLAY_LABEL);
                let mut foot_overlay_window =
                    app_handle.get_webview_window(FOOT_OVERLAY_LABEL);
                    ...
                    match reader.read_line(&mut line) {
                        ...
                        Ok(_) => {
                            ...
                            for (kind, cached_window) in [
                                (KeyViewerKind::Hand, &mut hand_overlay_window),
                                (KeyViewerKind::Foot, &mut foot_overlay_window),
                            ] {
                                let selected_mode = match kind {
                                ...
                                if let Some(overlay) = cached_window.as_ref() {
                                    let viewer_payload = json!({
                                        "key": key_label,
                                        "state": state,
                                        "mode": selected_mode,
                                    ...
                                    match overlay.emit("keys:state", &viewer_payload) {
                                        Ok(_) => emitted = true,
                                        Err(err) => {
                                            error!(
                                                "failed to emit keys:state to {}: {err}",
                                                overlay_label(kind)
                                            );
                                            *cached_window = None;
                                        }
    ...
    fn ensure_overlay_windows(&self, app: &AppHandle) -> Result<Vec<WebviewWindow>> {
        let mut windows = Vec::with_capacity(OVERLAY_KINDS.len());
        for kind in OVERLAY_KINDS {
            match self.ensure_overlay_window(app, kind) {
    ...
    fn ensure_overlay_window(
        ...
        let label = overlay_label(viewer_kind);
        ...
        let stored_layout = overlay_layout(&snapshot, viewer_kind);
        let paired_foot_bounds =
            if viewer_kind == KeyViewerKind::Foot && stored_layout.bounds.is_none() {
                overlay_layout(&snapshot, KeyViewerKind::Hand)
                    .bounds
                    .as_ref()
                    .map(|hand| OverlayBounds {
                        x: hand.x,
                        y: hand.y + hand.height + OVERLAY_PAIR_GAP,
        ...
        let is_paired_foot_position = paired_foot_bounds.is_some();
        let candidate_bounds = stored_layout.bounds.clone().or(paired_foot_bounds);
        let (mut bounds, had_bounds, mut bounds_are_logical) = if let Some(mut bounds) =
            candidate_bounds
        {
            let mut is_logical = stored_layout.bounds_are_logical
                || (viewer_kind == KeyViewerKind::Foot && stored_layout.bounds.is_none());
            if !is_logical {
        ...
        let position = if is_paired_foot_position {
            overlay_layout(&snapshot, KeyViewerKind::Hand)
                .bounds
                .as_ref()
                .map(|hand| self.compute_paired_foot_position(&bounds, hand, &monitor_data))
                .unwrap_or_else(|| {
                    self.compute_overlay_position(&bounds, had_bounds, &monitor_data)
                })
        } else {
            self.compute_overlay_position(&bounds, had_bounds, &monitor_data)
        };
        ...
        self.configure_overlay_window(&window, app, viewer_kind);

        // 위치가 보정되었거나 logical 플래그 동기화가 필요한 경우에만 store 갱신
        let needs_logical_sync = bounds_are_logical && !stored_layout.bounds_are_logical;
        if position_was_adjusted || !had_bounds || needs_logical_sync {
            if let Err(err) = self.store.update(|state| {
                let layout = overlay_layout_mut(state, viewer_kind);
                layout.bounds = Some(bounds.clone());
                layout.bounds_are_logical = bounds_are_logical;
            }) {
    ...
    fn configure_overlay_window(
        ...
        let initializing_flag = self.overlay_initializing_flag(viewer_kind);
                ...
                if force_close_flag.load(Ordering::SeqCst) {
                    ...
                    *overlay_visible.write() = false;
                    ...
                    if let Err(err) = state.set_overlay_visibility(&app_handle, false) {
                        log::error!("failed to hide overlay pair on close: {err}");
                    }
                }
            }
    ...
    fn compute_paired_foot_position(
        &self,
        foot: &OverlayBounds,
        hand: &OverlayBounds,
        monitors: &MonitorData,
        ...
        let Some(monitor) = monitors
            .find_best_overlap(hand.x, hand.y, hand.width, hand.height)
        ...
        let monitor_top = monitor.logical_origin_y;
        let monitor_bottom = monitor.logical_origin_y + monitor.logical_height;
        let below_y = hand.y + hand.height + OVERLAY_PAIR_GAP;
        let above_y = hand.y - OVERLAY_PAIR_GAP - foot.height;
        let preferred_y = if below_y + foot.height <= monitor_bottom {
            below_y
        } else if above_y >= monitor_top {
            above_y
        } else {
            below_y
        };
        monitor.clamp(hand.x, preferred_y, foot.width, foot.height)
    }
...
fn attach_main_window_close_handler(
    ...
    window.on_window_event(move |event| {
        if let WindowEvent::CloseRequested { api, .. } = event {
            ...
            for kind in OVERLAY_KINDS {
                let Some(overlay) = app_handle.get_webview_window(overlay_label(kind)) else {
                    continue;
                };
