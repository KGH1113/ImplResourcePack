...
use super::migration::{
    ...
    migrate_local_fonts_to_app_data, normalize_state, restore_missing_viewer_data_from_backup,
};
...
impl AppStore {
    pub fn initialize<R: Runtime>(resolver: &PathResolver<R>) -> Result<Self> {
        ...
        backup_pre_key_viewer_tabs_store(&default_path)?;
        let (path, mut state, mut needs_persist) = if default_path.exists() {
            ...
            backup_pre_key_viewer_tabs_store(&legacy_path)?;
        ...
        if !state.key_viewer_data_recovered {
            if restore_missing_viewer_data_from_backup(&path, &mut state)? {
                log::info!("[Store] Restored missing key viewer data from migration backup");
            }
            state.key_viewer_data_recovered = true;
            needs_persist = true;
        }
...
fn backup_pre_key_viewer_tabs_store(path: &Path) -> Result<()> {
    if !path.exists() {
        return Ok(());
    }
    let content = fs::read_to_string(path)
        .with_context(|| format!("failed to inspect store file at {}", path.display()))?;
    let already_migrated = serde_json::from_str::<serde_json::Value>(&content)
        .ok()
        .and_then(|value| {
            value
                .get("keyViewerTabsMigrated")
    ...
    let backup_path = path.with_file_name("store.pre-keyviewer-tabs.json");
    if !backup_path.exists() {
        fs::copy(path, &backup_path).with_context(|| {
            format!(
                "failed to back up pre-migration store from {} to {}",
                path.display(),
                backup_path.display()
            )
        })?;
