...
use crate::{
    ...
    models::{
        ...
        KeyViewerKind, KeyViewerTab, KnobPositions, NoteSettingsPatch, SettingsPatchInput,
        StatPositions,
    },
    state::{migration::migrate_legacy_ghost_keys, AppState},
};
...
pub fn preset_load(state: State<'_, AppState>, app: AppHandle) -> CmdResult<PresetOperationResult> {
    let picked = FileDialog::new()
        .add_filter("ImplDmNote Preset", &["json"])
        .pick_file();
    ...
    let ignored_legacy_tabs: Vec<&str> = super::LEGACY_BUILTIN_TAB_IDS
        .iter()
        .copied()
        .filter(|id| keys.contains_key(*id))
        .collect();
    if !ignored_legacy_tabs.is_empty() {
        log::warn!(
            "[Preset] Ignoring removed legacy built-in tabs: {}",
            ignored_legacy_tabs.join(", ")
        );
    }
    keys.retain(|id, _| !super::LEGACY_BUILTIN_TAB_IDS.contains(&id.as_str()));
    let mut positions = preset
        .key_positions
        .unwrap_or_else(|| default_positions().clone());
    migrate_legacy_ghost_keys(&mut positions);
    ...
    positions.retain(|id, _| !super::LEGACY_BUILTIN_TAB_IDS.contains(&id.as_str()));
    stat_positions.retain(|id, _| !super::LEGACY_BUILTIN_TAB_IDS.contains(&id.as_str()));
    graph_positions.retain(|id, _| !super::LEGACY_BUILTIN_TAB_IDS.contains(&id.as_str()));
    knob_positions.retain(|id, _| !super::LEGACY_BUILTIN_TAB_IDS.contains(&id.as_str()));
    let mut tabs = preset.tabs.unwrap_or_else(|| synthesize_tabs(&keys));
    tabs.retain(|tab| !super::LEGACY_BUILTIN_TAB_IDS.contains(&tab.id.as_str()));
    ensure_preset_viewer_tabs(&mut tabs);
    for tab in &tabs {
        keys.entry(tab.id.clone()).or_default();
        positions.entry(tab.id.clone()).or_default();
    }
    let snapshot = state.store.snapshot();
    let selected_key_type =
        choose_selected_key_type(preset.selected_key_type, &tabs, snapshot.selected_key_type);
    let mut selected_viewer_tabs = preset.selected_viewer_tabs.unwrap_or_default();
    selected_viewer_tabs.hand =
        selected_for_kind(&tabs, KeyViewerKind::Hand, Some(&selected_viewer_tabs.hand));
    selected_viewer_tabs.foot =
        selected_for_kind(&tabs, KeyViewerKind::Foot, Some(&selected_viewer_tabs.foot));
    if let Some(tab) = tabs.iter().find(|tab| tab.id == selected_key_type) {
        match tab.viewer_kind {
            KeyViewerKind::Hand => selected_viewer_tabs.hand = selected_key_type.clone(),
            KeyViewerKind::Foot => selected_viewer_tabs.foot = selected_key_type.clone(),
        }
    }
...
pub fn preset_load_tab(
    ...
    migrate_legacy_ghost_keys(&mut src_key_positions);
...
fn synthesize_tabs(keys: &KeyMappings) -> Vec<KeyViewerTab> {
    let mut index = 0usize;
    keys.keys()
        .filter(|key| !super::LEGACY_BUILTIN_TAB_IDS.contains(&key.as_str()))
        .map(|id| {
            index += 1;
            KeyViewerTab {
                id: id.clone(),
...
fn ensure_preset_viewer_tabs(tabs: &mut Vec<KeyViewerTab>) {
    if !tabs
        .iter()
        .any(|tab| tab.viewer_kind == KeyViewerKind::Hand)
    {
        tabs.push(KeyViewerTab {
            id: "hand-default".to_string(),
            name: "Hand 1".to_string(),
            viewer_kind: KeyViewerKind::Hand,
        });
    }
    if !tabs
    ...
    {
        tabs.push(KeyViewerTab {
            id: "foot-default".to_string(),
            name: "Foot 1".to_string(),
            viewer_kind: KeyViewerKind::Foot,
        });
    }
}

fn selected_for_kind(
    tabs: &[KeyViewerTab],
    kind: KeyViewerKind,
    preferred: Option<&str>,
) -> String {
    tabs.iter()
        .find(|tab| tab.viewer_kind == kind && preferred == Some(tab.id.as_str()))
        .or_else(|| tabs.iter().find(|tab| tab.viewer_kind == kind))
        .map(|tab| tab.id.clone())
        .expect("viewer tab must exist")
}
...
fn choose_selected_key_type(
    ...
    if let Some(req) = requested {
        if tabs.iter().any(|tab| tab.id == req) {
            return req;
        }
    }
    if tabs.iter().any(|tab| tab.id == fallback) {
        return fallback;
    }
    tabs.first()
        .map(|tab| tab.id.clone())
        .unwrap_or_else(|| "hand-default".to_string())
}
