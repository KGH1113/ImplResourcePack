...
export type TabResult = { result?: KeyViewerTab; error?: string };
export type TabDeleteResult = {
  success: boolean;
  selected: string;
  error?: string;
};
...
export type PluginDisplayElement = {
  ...
  tabId?: string; // 손·발 키뷰어 그룹에 속한 탭 ID
};
...
export interface DMNoteAPI {
  ...
  keys: {
    ...
    tabs: {
      ...
      onChanged(listener: (payload: TabsChangePayload) => void): Unsubscribe;
    };
  };
  statItems: {
