...
export const keyModeSchema = z.string().trim().min(1);
...
export const keyPositionSchema = z.object({
  ...
  keyLimiterExcluded: z.boolean().optional(),
...
export const keyViewerKindSchema = z.enum(['hand', 'foot']);
export type KeyViewerKind = z.infer<typeof keyViewerKindSchema>;

export const keyViewerTabSchema = z.object({
  id: z.string(),
  name: z.string().min(1),
  viewerKind: keyViewerKindSchema,
});
export type KeyViewerTab = z.infer<typeof keyViewerTabSchema>;

export const keyViewerTabsSchema = z.array(keyViewerTabSchema);

export interface SelectedViewerTabs {
  hand: string;
  foot: string;
}
