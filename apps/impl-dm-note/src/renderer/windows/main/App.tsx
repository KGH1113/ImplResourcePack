...
import { useKeyLimiterSync } from '@hooks/app/useKeyLimiterSync';
...
export default function App() {
  ...
  const { selectedKeyType, setSelectedKeyType, isBootstrapped, tabs } =
    useKeyStore();
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (!isBootstrapped || tabIds.length < 2) return;
      e.preventDefault();
      e.stopPropagation();
      const idx = Math.max(tabIds.indexOf(selectedKeyType), 0);
