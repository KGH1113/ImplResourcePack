...
function buildHistoryState(
  ...
  // 탭/선택 상태: 명시적 제공 시 사용, 없으면 현재 store에서 자동 캡처
  const keyState = useKeyStore.getState();
  const tabs = ('tabs' in input && input.tabs) || keyState.tabs;
  const selectedViewerTabs =
    ('selectedViewerTabs' in input && input.selectedViewerTabs) ||
    keyState.selectedViewerTabs;
  ...
  return {
    ...
    selectedViewerTabs: JSON.parse(JSON.stringify(selectedViewerTabs)),
    selectedKeyType,
    settingsSnapshot,
  };
}
