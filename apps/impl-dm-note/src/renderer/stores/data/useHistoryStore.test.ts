...
const DEFAULT_TABS: KeyViewerTab[] = [
  { id: 'tab-1', name: 'Tab 1', viewerKind: 'hand' },
  { id: 'tab-2', name: 'Tab 2', viewerKind: 'foot' },
];

const resetStores = () => {
  useHistoryStore.setState({ past: [], future: [] });
  useKeyStore.setState({
    selectedKeyType: 'tab-1',
    tabs: DEFAULT_TABS,
    selectedViewerTabs: { hand: 'tab-1', foot: 'tab-2' },
...
describe('useHistoryStore', () => {
  ...
  it('pushState는 현재 카운터 캐시 스냅샷을 즉시 저장', () => {
    applyCounterCacheSnapshot({ 'tab-1': { A: 3 } });
    ...
    setCachedKeyCounter('tab-1', 'A', 9);

    const history = useHistoryStore.getState().past;
    expect(history).toHaveLength(1);
    expect(history[0]?.keyCounters).toEqual({ 'tab-1': { A: 3 } });
  });

  it('undo는 현재 카운터 캐시를 future에 저장하고 이전 스냅샷을 반환', () => {
    applyCounterCacheSnapshot({ 'tab-1': { A: 2 } });
    useHistoryStore.getState().pushState({
      keyMappings: createMappings('A'),
      positions: createPositions(1),
      statPositions: EMPTY_STATS,
    ...
    applyCounterCacheSnapshot({ 'tab-1': { A: 7 } });
    ...
    expect(restored?.keyCounters).toEqual({ 'tab-1': { A: 2 } });
    ...
    expect(future[0]?.keyCounters).toEqual({ 'tab-1': { A: 7 } });
  });
  ...
  it('redo는 future의 스냅샷을 복원하고 현재 카운터 캐시를 past에 저장', () => {
    ...
    applyCounterCacheSnapshot({ 'tab-1': { A: 4 } });
