...
import type { KeyViewerKind, SelectedViewerTabs } from '@src/types/key/keys';
...
export function resolveViewerSelectedKeyType(
  viewerKind: KeyViewerKind | undefined,
  selectedViewerTabs: SelectedViewerTabs,
  selectedKeyType: string,
) {
  return viewerKind ? selectedViewerTabs[viewerKind] : selectedKeyType;
}

export function mergeSelectedViewerTabs(
  current: SelectedViewerTabs,
  incoming: SelectedViewerTabs,
  changedViewerKind?: KeyViewerKind,
): SelectedViewerTabs {
  if (!changedViewerKind) return incoming;
  return {
    ...current,
    [changedViewerKind]: incoming[changedViewerKind],
  };
}
...
export function useAppBootstrap(viewerKind?: KeyViewerKind) {
  useEffect(() => {
    ...
    const applyResyncSnapshot = (bootstrap: BootstrapPayload) => {
      ...
      if (stableStringify(keyState.tabs) !== stableStringify(bootstrap.tabs)) {
        keyChanges.tabs = bootstrap.tabs;
      }
      if (
        stableStringify(keyState.selectedViewerTabs) !==
        stableStringify(bootstrap.selectedViewerTabs)
      ) {
        keyChanges.selectedViewerTabs = bootstrap.selectedViewerTabs;
      }
      const selectedKeyType = resolveViewerSelectedKeyType(
        viewerKind,
        bootstrap.selectedViewerTabs,
        bootstrap.selectedKeyType,
      );
      if (keyState.selectedKeyType !== selectedKeyType) {
        keyChanges.selectedKeyType = selectedKeyType;
      }
    ...
    (async () => {
      try {
        ...
        useKeyStore.setState((state) => ({
          ...
          tabs: bootstrap.tabs,
          selectedViewerTabs: bootstrap.selectedViewerTabs,
          selectedKeyType: resolveViewerSelectedKeyType(
            viewerKind,
            bootstrap.selectedViewerTabs,
            bootstrap.selectedKeyType,
          ),
    ...
    const unsubscribers = [
      ...
      window.api.keys.tabs.onChanged(
        ({ tabs, selectedKeyType, selectedViewerTabs, changedViewerKind }) => {
          useKeyStore.setState((state) => {
            const mergedViewerTabs = mergeSelectedViewerTabs(
            ...
            return {
              ...
              selectedViewerTabs: mergedViewerTabs,
            };
          });
        },
      ),
      ...
      window.api.presets.onSnapshot((snapshot) => {
        ...
        useKeyStore.setState((state) => ({
          ...
          tabs: snapshot.tabs,
          selectedViewerTabs: snapshot.selectedViewerTabs,
          selectedKeyType: resolveViewerSelectedKeyType(
            viewerKind,
            snapshot.selectedViewerTabs,
