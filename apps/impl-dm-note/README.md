...
ImplDmNote is an independent GPL-3.0-only desktop key viewer based on DM Note v1.6.1. It uses its own application identity and data directory and synchronizes the active key profile with ImplResourcePack through ADOFAI-IPC.

This is a modified version and is not an official DM Note release. See [UPSTREAM.md](UPSTREAM.md) for source provenance and modifications. The original v1.6.1 documentation follows below for feature and usage reference; its release and update links do not apply to ImplDmNote.
